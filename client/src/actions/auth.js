...
export const register = ({name, email, password}) => async dispatch => {
    ...
    try {
        const res = await axios.post('http://localhost:5002/api/v1/auth/register', body, config);
        console.log(res);
        dispatch({
...
export const login = (email, password) => async dispatch => {
    ...
    try {
        const res = await axios.post('http://localhost:5002/api/v1/auth/login', body, config);
        console.log(res);
        dispatch({
            type: LOGIN_SUCCESS,
            payload: res.data
